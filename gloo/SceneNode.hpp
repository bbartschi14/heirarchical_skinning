...
class SceneNode {
 public:
  ...
  std::vector<SceneNode*> GetGizmoPtrs() {
      return gizmo_spheres_;
  }
  void AddToGizmoPtrs(SceneNode* ptr) {
      gizmo_spheres_.push_back(ptr);
  }
  int GetGizmoAxis() {
      return axis_;
  }
  void SetGizmoAxis(int axis) {
      axis_ = axis;
  }
 ...
 private:
  ...
  SceneNode* parent_;
  std::vector<SceneNode*> gizmo_spheres_;
  int axis_;
  bool active_;
};
}  // namespace GLOO
